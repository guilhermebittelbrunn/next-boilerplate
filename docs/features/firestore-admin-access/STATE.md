...
| test    | done    | 2026-08-30 23:05 | test/report.md | **+33 testes na api (72 → 105), root verde (242), typecheck limpo, `pnpm check` 192/37 sem regressão.** As 7 lacunas do handoff/review fechadas — inclusive a **6**, provada em 3 camadas: unit com `Timestamp` **real**, de rota (`/auth/me`, `/users/:id`) e **medida em runtime** (todos ISO, zero `_seconds`). Todos os testes novos provados load-bearing por mutação. Ownership medido com **dois usuários comuns reais**: entidade alheia → **404 `ENTITY_NOT_FOUND`**; sem credencial → 401; admin sem personificar → 403. CRUD + admin + impersonação redirigidos no browser (14 prints, PII filtrada por `qa-`). 🔴 **1 defeito novo encontrado E corrigido nesta etapa**: `serializeFirestoreValue` não tratava `Date` do JS, então o perfil criado dentro do request saía com `createdAt: {}` em `GET /auth/me` e `POST /auth/sign-in` — **regressão introduzida por `84201b6`**, reproduzida em runtime, corrigida e travada por teste (removida a correção, só ele falha). ⚠️ **2 critérios seguem BLOQUEADO — depende de ação humana**: `:111` e `:113` exigem as rules publicadas; leitura REST com a chave pública medida agora ainda devolve **HTTP 200 com dado real**. Não é falha da implementação. Correção + testes **no working tree, não commitados** |
